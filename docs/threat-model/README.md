...
| T-20 | Denial of service | Huge or malicious documents exhaust extraction | ai-service | Request size cap (200k chars), timeouts, low-confidence HOLD on failure; OCR checks the pixel count from the image header before decoding, runs Tesseract and Poppler without a shell, single-threaded, with a 30 s timeout and an environment holding no secrets (ADR-0018) | AI_EXTRACTION_LOW_CONFIDENCE, ADR-0018 | Phase 0 contract |
